...
typedef struct storable_picture
{
  ...
  imgpel **     imgY_ups_aif;      //!< Y picture component upsampled with new calculated filter coeffitients
  double **     imgY_ups_aif_hor;  //!< Y picture component upsampled with new calculated horizontal filter coefficients (separable aif) 
  imgpel **     p_imgY_ups_aif;    //!< pointer to imgY_ups or to imgY_ups_aif

#ifdef E_DAIF
  imgpel *    imgY_11_aif;       //!< Y picture component with padded borders filtered with new calcualted filter coefficients
  imgpel *    p_imgY_11_aif;    //!< pointer to imgY_11 or to imgY_11_aif
#endif

#endif

#ifdef SWITCHED_FILTERS
  imgpel **   imgY_filt;          //!< Y picture component
  imgpel *    imgY_11_filt;       //!< Y picture component with padded borders
  imgpel *    imgY_11_w_filt;     //!< Y picture component with padded borders for weighted prediction
  imgpel **   p_imgY_filt;  
  imgpel *    p_imgY_11_filt;
  imgpel *    p_imgY_11_w_filt;  
...
#ifdef ADAPTIVE_FILTER
  double      FilterCoefficients[15][36];
  int         slice_type;
#endif
} StorablePicture;
