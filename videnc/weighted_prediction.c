...
int test_wp_B_slice(int select_method)
{
  ...
  if (select_method == 0) //! implicit mode
  {
...
#ifdef  BUG_FIX_FOR_FREXT
    active_refs[1]=input->B_List1_refs == 0 ? listXsize[1] : min(input->B_List1_refs,listXsize[1]);
#else
    ...
    for (clist=0; clist<2 + list_offset; clist++)
    {
      for (index = 0; index < active_refs[clist]; index++)
      {
        ...
        {
#ifdef USE_NEW_OFFSET
          if (wp_weight[clist][index][comp] != default_weight[comp] || (wp_offset[clist][index][comp] != 0 && input->UseNewOffset) )
#else
  ...
  if (input->SimplifiedRDPicDecision && (select_method == 1))    
  {
    perform_wp = 1;
    if(	wbp_weight[1][0][0][0] == 32 && wbp_weight[0][0][0][0] == 32 )
      perform_wp = 0;
  }
#endif
