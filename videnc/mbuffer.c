...
StorablePicture* alloc_storable_picture(PictureStructure structure, int size_x, int size_y, int size_x_cr, int size_y_cr)
{
  ...
  if(input->UseHPFilter == HPF_SIFO_FPO)
  {
    s->imgY_11 = malloc ((size_x * size_y) * sizeof (imgpel));
    if (NULL == s->imgY_11)
      no_mem_exit("alloc_storable_picture: s->imgY_11");
    get_mem2Dpel (&(s->imgY_ups), (2*IMG_PAD_SIZE + size_y)*4, (2*IMG_PAD_SIZE + size_x)*4);
  }
  else
  {
...
#endif
#ifdef USE_HP_FILTER
      if(input->UseHPFilter==2)
        UnifiedOneForthPixEnhancedFDIF(p);
      else if(input->UseHPFilter==1)
        UnifiedOneForthPixWithHighPrecisionH264Filter_16(p); 
#ifdef SWITCHED_FILTERS
      else if((input->UseHPFilter == HPF_SIFO) || (input->UseHPFilter == HPF_SIFO_FPO))
      {
...
void dpb_split_field(FrameStore *fs)
{
  ...
  if (!active_sps->frame_mbs_only_flag)
  {
    ...
    if( img->mv_res )
    {
      oneeighthpix(fs->top_field);
      oneeighthpix(fs->bottom_field);
    }
    else
    {
#endif
      ...
      if(input->UseHPFilter==2)
      {
        UnifiedOneForthPixEnhancedFDIF(fs->top_field);
        UnifiedOneForthPixEnhancedFDIF(fs->bottom_field);
      }
      else if(input->UseHPFilter==1)
      {
        UnifiedOneForthPixWithHighPrecisionH264Filter_16(fs->top_field);
        UnifiedOneForthPixWithHighPrecisionH264Filter_16(fs->bottom_field);
      }
#ifdef SWITCHED_FILTERS
      else if((input->UseHPFilter == HPF_SIFO) || (input->UseHPFilter == HPF_SIFO_FPO))
      {
...
#ifdef EIGHTH_PEL
  if( img->mv_res )
    oneeighthpix(fs->frame);
  else
#endif
#ifdef USE_HP_FILTER
    if(input->UseHPFilter==2)
      UnifiedOneForthPixEnhancedFDIF(fs->frame);
    else if(input->UseHPFilter==1)
      UnifiedOneForthPixWithHighPrecisionH264Filter_16(fs->frame);
#ifdef SWITCHED_FILTERS
    else if((input->UseHPFilter == HPF_SIFO) || (input->UseHPFilter == HPF_SIFO_FPO))
      error ("Switched filters are not compatible with interlace coding", 0);
    else
