...
static void PatchInp (void)
{
  ...
  */
...
#ifdef MB32X32
  if(input->UseExtMB > 0 && input->AdaptiveRounding == 1)
    error("adaptive rounding is not supported currently in bigger motion partition", -1);
  if(input->UseExtMB > 0 && input->symbol_mode==UVLC)
    error("UVLC is not supported currently in bigger motion partition", -1);
#endif
  ...
  if(((input->UseHPFilter == HPF_SIFO) || (input->UseHPFilter == HPF_SIFO_FPO)) 
    && (input->PicInterlace > 0) && (input->MbInterlace > 0))
      error ("Switched filters are not compatible with interlace coding", 0);
#endif

#ifdef SIMPLIFIED_RDPIC_DECISION
  if((input->SimplifiedRDPicDecision == 1) && ((input->UseHPFilter == HPF_SIFO) || (input->UseHPFilter == HPF_SIFO_FPO)))
      fprintf(stderr, "Warning: SimplifiedRDPicDecision has no effect when Switched Filters are used\n");
#endif
