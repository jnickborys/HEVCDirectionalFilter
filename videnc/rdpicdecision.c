...
int picture_coding_decision (Picture *picture1, Picture *picture2, int qp)
{
  ...
  if(input->UseAdaptiveQuantMatrix)
    lambda_picture = img->lambda_md[img->type][gdwBaseQp];
  else
#endif
 ...
 ************************************************************************
 * \brief
 *    RD decision between possible encoding cases for AQMS
 ************************************************************************
 */
int rd_pic_decision_for_AQMS (double snrY_version1, double snrY_version2, int bits_version1, int bits_version2, double lambda_picture)
{
  double cost_version1, cost_version2;
  ...
  if (( snrY_version2 >= snrY_version1) )
    return (0);
  else
    return (1);
}
 ...
 ************************************************************************
 * \brief
 *    Picture Coding Decision for AQMS
 ************************************************************************
 */
int picture_coding_decision_for_AQMS (Picture *picture1, Picture *picture2, int qp)
{
  ...
  if((img->type==I_SLICE || img->currentPicture->idr_flag))
    return rd_pic_decision_for_AQMS(snr_picture1, snr_picture2, bit_picture1, bit_picture2, lambda_picture);
  else
    return rd_pic_decision(snr_picture1, snr_picture2, bit_picture1, bit_picture2, lambda_picture);
}
