...
#ifdef MB32X32
int find_SATD32 (int c_diff[MB_PIXELS], int blocktype, int mb_ext_level);
...
#ifdef MB32X32
    if(input->UseExtMB > 0)
      memory_size += get_mem6Dshort (&EPZSMotion, 6, img->max_num_references, 7, 4<<MAX_MB_EXT_LEVEL, img->width/BLOCK_SIZE, 2);
    else
#endif
...
#ifdef MB32X32
    if(input->UseExtMB > 0)
      free_mem6Dshort (EPZSMotion, 6, img->max_num_references, 7, 4<<MAX_MB_EXT_LEVEL);
    else
#endif
                                      ...
                                      EPZSStructure * predictor)
{
  ...
  *prednum += ((predictor->point[*prednum].x != 0) || (predictor->point[*prednum].y != 0));
  ...
  if(input->UseExtMB > 0)
  {
    predictor->point[*prednum].x = (by > 0) ? mv[by - bs_y][pic_x][0] : mv[max(4 - bs_y,0)][pic_x][0] ;
    predictor->point[*prednum].y = (by > 0) ? mv[by - bs_y][pic_x][1] : mv[max(4 - bs_y,0)][pic_x][1];
    *prednum += ((predictor->point[*prednum].x != 0) || (predictor->point[*prednum].y != 0));
    // Up-Right predictor
    predictor->point[*prednum].x = (pic_x + bs_x < img_width) ? (by > 0) 
      ? mv[by - bs_y][pic_x + bs_x][0] : mv[max(4 - bs_y,0)][pic_x + bs_x][0] : 0;
    predictor->point[*prednum].y = (pic_x + bs_x < img_width) ? (by > 0) 
      ? mv[by - bs_y][pic_x + bs_x][1] : mv[max(4 - bs_y,0)][pic_x + bs_x][1] : 0;
    *prednum += ((predictor->point[*prednum].x != 0) || (predictor->point[*prednum].y != 0));
  }
  else
  {
                      ...
                      int cand_y)
{
  ...
  for (y=0; y<blocksize_y; y++)
  {
    ...
    for (x4 = 0; x4 < blockshape_x; x4++)
    {
      pixel1=(*cur_line++);
      pixel2=(*ref_line++);
      diff = pixel1  - Clip3 (0, img->max_imgpel_value , pixel2 + img->currMEOffset[list]);
      mcost += byte_abs[diff];
...
************************************************************************
* \brief
*    BiPred SAD computation
************************************************************************
*/
static int computeBiPredSad3(pel_t** cur_pic,
                             int blocksize_y,
                             int blocksize_x, 
                             int blockshape_x,
                             int mcost,
                             ...
                             int cand_x2, int cand_y2)
{
...
#else
  round = img->currMEOffset[0] + img->currMEOffset[1];
#endif
  ...
  for (y = 0; y < blocksize_y; y++)
  {
    ...
    for (x4 = 0; x4 < blockshape_x; x4++)
    {         
      bi_diff = (*cur_line++) - clip1a((*ref1_line++ + *ref2_line++ + round) >> 1);
                          ...
                          int lambda_factor)      // <--  lagrangian parameter for determining motion cost
{
...
#ifdef SWITCHED_FILTERS
  min_mcost = computeSad(cur_pic, list, blocksize_y,blocksize_x,
  ...
  {    
    ...
    for (pos = 0; pos < prednum; pos++)
    {
      ...
      get_ref_line = CHECK_RANGE ? FastLineX : UMVLineX;      
#ifdef SWITCHED_FILTERS
      mcost = computeSad(cur_pic, list, blocksize_y,blocksize_x, 
  ...
  {
#if EPZSREF  
    if (input->EPZSSpatialMem && (img->currMEOffset[list] == 0))  
#else
    if (input->EPZSSpatialMem && (img->currMEOffset[list] == 0) && (ref == 0))  
#endif
    {
                             ...
                             int    lambda_factor)        // <--  lagrangian parameter for determining motion cost
{
  ...
  {
    if(apply_weights)
      error("Single Pass Encoding does not support weighted prediction.", 0);
    else
      computeBiPredSad = computeBiPredSad3;
  }
  else
  {
...
#ifdef SWITCHED_FILTERS
  short mv_x_offset = *mv_x;
  short mv_y_offset = *mv_y;      
#endif
  ...
  *********************************/
...
#ifdef SWITCHED_FILTERS
  if(((img->filterParam == SIFO_FIRST_PASS_FPO) && (img->currMEOffset[list] == 0)) ||
  ...
  for (best_pos = 0, pos = min_pos2; pos < 5; pos++)
  {
    ...
    if (input->mv_competition > 0)
      mcost = compute_mv_cost_for_each_predictor (lambda_factor, mv_shift, cand_mv_x, cand_mv_y, pred_mv_x, pred_mv_y, ref, list, blocktype, pic_pix_x-img->opix_x, pic_pix_y-img->opix_y);
    else
    ...
    /*#ifdef MV_COMPETITION*/
    //    mcost = compute_mv_cost_for_each_predictor (lambda_factor, 0, cand_mv_x, cand_mv_y, pred_mv_x, pred_mv_y, ref, list, blocktype, pic_pix_x-img->opix_x, pic_pix_y-img->opix_y);
    //#else
...
#ifdef SWITCHED_FILTERS
  if((img->filterParam == SIFO_FIRST_PASS_FPO) && (ref == 0))
  {
    best_pos = -1;
    get_line = UMVLine4X;   

    for(pos = 0; pos < img->noOffsets[list]; pos++)
    {
      cand_mv_x = mv_x_offset + img->search_point_qp_x_offset[pos];    // quarter-pel units
      cand_mv_y = mv_y_offset + img->search_point_qp_y_offset[pos];    // quarter-pel units
    ...
    if (best_pos>=0){
      *mv_x = mv_x_offset + img->search_point_qp_x_offset[best_pos];    // quarter-pel units
      *mv_y = mv_y_offset + img->search_point_qp_y_offset[best_pos];    // quarter-pel units
    }
  }
#endif
...
{
  StorablePicture *ref_picture = listX[list+list_offset][ref];
  short blocksize_y = input->blc_size[blocktype][1]<<mb_ext_level;  // vertical block size
  short blocksize_x = input->blc_size[blocktype][0]<<mb_ext_level;  // horizontal block size
  ...
  int   pred_x = (pic_pix_x << 2/*integer->subpel*/) + pred_mv_x;  // predicted position x (in sub-pel units)
  int   pred_y = (pic_pix_y << 2/*integer->subpel*/) + pred_mv_y;  // predicted position y (in sub-pel units)
  ...
  int   tempmv_x2 = 0, tempmv_y2 = 0;
#ifdef SPEED_UP
  int   stopCriterion = medthres[blocktype]*(1<<(2*mb_ext_level));
#else
  int   stopCriterion = medthres[blocktype];
#endif
  int   mapCenter_x = search_range - *mv_x;
  ...
  if ((ref>0 && img->structure == FRAME)  
#ifdef SPEED_UP
    && (prevSad[pic_pix_x2] < medthres[blocktype]*(1<<(2*mb_ext_level))) 
#else
    && (prevSad[pic_pix_x2] < medthres[blocktype]) 
#endif
    && (prevSad[pic_pix_x2] < min_mcost))
  ...
  if (min_mcost > stopCriterion)
  {    
    int mb_available_right   = (img->mb_x < (img_width  >> 4) - (1<<mb_ext_level));
    int mb_available_below   = (img->mb_y < (img_height >> 4) - (1<<mb_ext_level));
    int sadA, sadB, sadC;    
    int block_available_right;
    int block_available_below;
    int prednum = 5;
    ...
    
    {
      ...
      reset_mb_nr = ResetAvailabilityOfNeighbors32(mb_x, mb_y, &reset_block_x, &reset_block_y, blocksize_x, blocksize_y);
      ...
      getLuma4x4Neighbour(reset_mb_nr, reset_block_x, reset_block_y,  blocksize_x, -1, &block_c);
    ...
    if (mb_y > 0)
    {
      ...
      {
        if (mb_y == (8<<mb_ext_level))    //lower 32x16
        {
          block_available_right = (blocksize_x != (MB_BLOCK_SIZE<<mb_ext_level)) || mb_available_right;

          if (blocksize_x == (MB_BLOCK_SIZE<<mb_ext_level))
            block_c.available = 0;
        }
        else              //never
        {
          block_available_right = (mb_x + blocksize_x != (8<<mb_ext_level)) || mb_available_right;

          if (mb_x + blocksize_x == (8<<mb_ext_level))
            block_c.available = 0;
        }
      }
      else               //never
      {
        block_available_right = (mb_x + blocksize_x != (MB_BLOCK_SIZE<<mb_ext_level)) || mb_available_right;

        if (mb_x + blocksize_x == (MB_BLOCK_SIZE<<mb_ext_level))   
          block_c.available = 0;
      }
    }
    else  //16x32, upper 32x16, 32x32
    {
      block_available_right = (mb_x + blocksize_x != (MB_BLOCK_SIZE<<mb_ext_level)) || mb_available_right;
    }    
    block_available_below = (mb_y + blocksize_y != (MB_BLOCK_SIZE<<mb_ext_level)) || (mb_available_below);    
    ...
    stopCriterion = min(sadA,min(sadB,sadC));
#ifdef SPEED_UP
    stopCriterion = max(stopCriterion,minthres[blocktype]*(1<<(2*mb_ext_level)));
    stopCriterion = min(stopCriterion,maxthres[blocktype]*(1<<(2*mb_ext_level)));
    
    stopCriterion = (9 * max (medthres[blocktype]*(1<<(2*mb_ext_level)), stopCriterion) + 2 * medthres[blocktype]*(1<<(2*mb_ext_level))) >> 3;
#else
    stopCriterion = max(stopCriterion,minthres[blocktype]);
    stopCriterion = min(stopCriterion,maxthres[blocktype]);
    ...
    if (min_mcost > stopCriterion)
    {
      ...
      {
#ifdef SPEED_UP
        if ((min_mcost < stopCriterion + ((3 * medthres[blocktype]*(1<<(2*mb_ext_level))) >> 1)))
#else
        if ((min_mcost < stopCriterion + ((3 * medthres[blocktype]) >> 1)))
#endif
        {
...
#else
  computeBiPredSad = apply_weights? computeBiPredSad2 : computeBiPredSad1;//scalable to MB32X32
#endif
  ...
  {       
    ...
    if (mb_y > 0)
    {
      if (mb_x < (8<<mb_ext_level))    // first column of 8x8 blocks
      {
        if (mb_y==16)
        {
          if (blocksize_x == (MB_BLOCK_SIZE<<mb_ext_level)) //lower 32x16
            block_c.available  = 0;
        }
        else if (mb_x+blocksize_x == (8<<mb_ext_level))      //left 16x32
          block_c.available = 0;
      }
      else if (mb_x+blocksize_x == (MB_BLOCK_SIZE<<mb_ext_level)) //the 4th 16x16
        block_c.available = 0;
    }
#ifdef SPEED_UP
    stopCriterion = (11 * medthres[blocktype]*(1<<(2*mb_ext_level))) >> 3;
#else
    stopCriterion = (11 * medthres[blocktype]) >> 3;
#endif
    //! Add Spatial Predictors in predictor list.
                             ...
                             int       mb_ext_level )
{
  ...
  int   halfpelhadamard  = input->hadamard == 2 ? 0 : input->hadamard;
  int   qpelstart        = input->hadamard == 2 ? 0 : 1;
  int   test8x8transform = input->Transform8x8Mode && blocktype <= 7 && halfpelhadamard;
  int   cmv_x, cmv_y;
    ...
    for (y0=0, abort_search=0; y0<blocksize_y && !abort_search; y0+=4)
    {
      ...
      for (x0=0; x0<blocksize_x; x0+=BLOCK_SIZE)
      {
        ...
        *d        = *orig_line   - *(ref_line += 4);
        ...
        {
          //convert 8x8 array into 1-D array
          int x8 = x0/8, y8=y0/8;
