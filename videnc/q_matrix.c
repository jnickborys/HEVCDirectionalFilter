...
int *****LevelScale4x4Luma_IAQMS;
int ******LevelScale4x4Chroma_IAQMS;
int *****LevelScale8x8Luma_IAQMS;

int *****InvLevelScale4x4Luma_IAQMS;
int ******InvLevelScale4x4Chroma_IAQMS;
int *****InvLevelScale8x8Luma_IAQMS;

short ScalingList4x4_IAQMS[NUM_OF_IAQMS_MAT][6][16];
short ScalingList8x8_IAQMS[NUM_OF_IAQMS_MAT][2][64];
#endif
...
void allocate_QMatrix ()
{
  ...
  {
    get_mem5Dint(&LevelScale4x4Luma_IAQMS,    NUM_OF_IAQMS_MAT, 2, 6, 4, 4);
    get_mem5Dint(&LevelScale8x8Luma_IAQMS,    NUM_OF_IAQMS_MAT, 2, 6, 8, 8);
    get_mem6Dint(&LevelScale4x4Chroma_IAQMS,  NUM_OF_IAQMS_MAT, 2, 2, 6, 4, 4);

    get_mem5Dint(&InvLevelScale4x4Luma_IAQMS, NUM_OF_IAQMS_MAT, 2, 6, 4, 4);
    get_mem5Dint(&InvLevelScale8x8Luma_IAQMS, NUM_OF_IAQMS_MAT, 2, 6, 8, 8);
    get_mem6Dint(&InvLevelScale4x4Chroma_IAQMS, NUM_OF_IAQMS_MAT, 2, 2, 6, 4, 4);
  }
#endif
...
void free_QMatrix ()
{
  ...
  if(input->UseAdaptiveQuantMatrix)
  {
    free_mem5Dint(LevelScale4x4Luma_IAQMS, NUM_OF_IAQMS_MAT, 2, 6);
    free_mem5Dint(LevelScale8x8Luma_IAQMS, NUM_OF_IAQMS_MAT, 2, 6);
    free_mem6Dint(LevelScale4x4Chroma_IAQMS, NUM_OF_IAQMS_MAT, 2, 2, 6);

    free_mem5Dint(InvLevelScale4x4Luma_IAQMS, NUM_OF_IAQMS_MAT, 2, 6);
    free_mem5Dint(InvLevelScale8x8Luma_IAQMS, NUM_OF_IAQMS_MAT, 2, 6);
    free_mem6Dint(InvLevelScale4x4Chroma_IAQMS, NUM_OF_IAQMS_MAT, 2, 2, 6);
  }
#endif
      ...
      for(k=0; k<6; k++)
      {
        for(j=0; j<4; j++)
        {
          for(i=0; i<4; i++)
          {
            temp = (j<<2)+i;
            // Intra
            LevelScale4x4Luma_IAQMS[l][1][k][j][i]         = (quant_coef[k][j][i]<<4)/ScalingList4x4_IAQMS[l][0][temp];
            InvLevelScale4x4Luma_IAQMS[l][1][k][j][i]      = dequant_coef[k][j][i]*ScalingList4x4_IAQMS[l][0][temp];

            LevelScale4x4Chroma_IAQMS[l][0][1][k][j][i]    = (quant_coef[k][j][i]<<4)/(ScalingList4x4_IAQMS[l][1][temp]);
            InvLevelScale4x4Chroma_IAQMS[l][0][1][k][j][i] = dequant_coef[k][j][i]*(ScalingList4x4_IAQMS[l][1][temp]);

            LevelScale4x4Chroma_IAQMS[l][1][1][k][j][i]    = (quant_coef[k][j][i]<<4)/(ScalingList4x4_IAQMS[l][2][temp]);
            InvLevelScale4x4Chroma_IAQMS[l][1][1][k][j][i] = dequant_coef[k][j][i]*(ScalingList4x4_IAQMS[l][2][temp]);

            // Inter
            LevelScale4x4Luma_IAQMS[l][0][k][j][i]         = (quant_coef[k][j][i]<<4)/ScalingList4x4_IAQMS[l][3][temp];
            InvLevelScale4x4Luma_IAQMS[l][0][k][j][i]      = dequant_coef[k][j][i]*ScalingList4x4_IAQMS[l][3][temp];

            LevelScale4x4Chroma_IAQMS[l][0][0][k][j][i]    = (quant_coef[k][j][i]<<4)/(ScalingList4x4_IAQMS[l][4][temp]);
            InvLevelScale4x4Chroma_IAQMS[l][0][0][k][j][i] = dequant_coef[k][j][i]*(ScalingList4x4_IAQMS[l][4][temp]);

            LevelScale4x4Chroma_IAQMS[l][1][0][k][j][i]    = (quant_coef[k][j][i]<<4)/(ScalingList4x4_IAQMS[l][5][temp]);
            InvLevelScale4x4Chroma_IAQMS[l][1][0][k][j][i] = dequant_coef[k][j][i]*(ScalingList4x4_IAQMS[l][5][temp]);
          }
        }
      }
    }
      ...
      for(k=0; k<6; k++)
      {
        for(j=0; j<8; j++)
        {
          for(i=0; i<8; i++)
          {
            ...
            // Intra
            temp = (i<<3)+j;
            LevelScale8x8Luma_IAQMS[l][1][k][j][i]    = (quant_coef8[k][j][i]<<4)/ScalingList8x8_IAQMS[l][0][temp];
            InvLevelScale8x8Luma_IAQMS[l][1][k][j][i] = dequant_coef8[k][j][i]*ScalingList8x8_IAQMS[l][0][temp];

            // Inter
            LevelScale8x8Luma_IAQMS[l][0][k][j][i]    = (quant_coef8[k][j][i]<<4)/ScalingList8x8_IAQMS[l][1][temp];
            InvLevelScale8x8Luma_IAQMS[l][0][k][j][i] = dequant_coef8[k][j][i]*ScalingList8x8_IAQMS[l][1][temp];
