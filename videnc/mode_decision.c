...
#if defined RDO_Q && defined MB32X32 //SPEEDUP
extern int curr_mbi;
#endif
/*!
*************************************************************************************
...
#ifdef MB32X32
void rc_store_diff64(int cpix_x, int cpix_y, imgpel prediction[16<<MAX_MB_EXT_LEVEL][16<<MAX_MB_EXT_LEVEL])
{
  int i,j;
  int pix_x, pix_y;
...
#ifdef MB32X32
  if(input->UseExtMB>0)
    enc_mb->valid[0]     = (!intra && input->InterSearchSkip);
  else
#endif
...
// compatibility bug fix
  if(input->UseAdaptiveQuantMatrix && (img->slice_fractional_quant_flag || (input->UseRDO_Q && final_mb_encoding == 1)))
// compatibility bug fix
    SetModeAvalability(enc_mb);
  else
#endif
  if(input->UseRDO_Q && final_mb_encoding == 1)
  {
    int i;
    for(i=0; i<MAXMODE; i++)
      enc_mb->valid[i] = 0;
    
    enc_mb->valid[saved_best_mode] = 1;
  ...
  if(input->UseAdaptiveQuantMatrix)
  {
    ...
    if(input->UseRDO_Q)
    {
      ...
      { 
        ...
        enc_mb->lambda_me = img->lambda_me[5][img->masterQP];
        enc_mb->lambda_mf = img->lambda_mf[5][img->masterQP];
      }
      else
      {
        enc_mb->lambda_md = img->lambda_md[img->type][img->masterQP];
        enc_mb->lambda_me = img->lambda_me[img->type][img->masterQP];
        enc_mb->lambda_mf = img->lambda_mf[img->type][img->masterQP];
      }
    }
    else
    {
...
// SPEEDUP
#if defined RDO_Q && defined MB32X32
  if(input->UseExtMB>0 && clusterof16_isbetterthan_one32_duringRDOQ == 1 && Motion_Selected == 1)
  {
    int i,mode;
    for(i=0; i<MAXMODE; i++)
      enc_mb->valid[i] = 0;
  
    mode = saveMode.MBsize16[curr_mbi];
  ...
  if (currMB->c_ipred_mode == DC_PRED_8 || (IS_INTRA(currMB) ))
  {
    while(1)
    {
      ...
      if (input->Transform8x8Mode==1)
      {
        ...
        else if (input->UseExtMB>0 && (mode == 1) && currMB->luma_transform_size_8x8_flag == 1)
        {
        ...
        // try partition 16x16 with 16x16 transform
        else if (input->UseExtMB>0 && (mode == 2) && currMB->luma_transform_size_8x8_flag == 1)
        {
          //try with 16x16 transform size
          currMB->luma_transform_size_8x8_flag = 2;
          continue;
        ...
        // try partition 16x16 with 16x16 transform
        else if (input->UseExtMB>0 && (mode == 3) && currMB->luma_transform_size_8x8_flag == 1)
        {
          //try with 16x16 transform size
          currMB->luma_transform_size_8x8_flag = 2;
          continue;
...
#ifdef MB32X32
        else if (input->UseExtMB>0 && mode == 0 && bslice && active_sps->direct_8x8_inference_flag && currMB->luma_transform_size_8x8_flag == 1)
        {
          //try with 16x16 transform size
          currMB->luma_transform_size_8x8_flag = 2;
          continue;
  ...
  for (min_cost8x8=INT_MAX, min_rdcost=1e30, index=(bslice?0:1); index<maxindex; index++)
  {
    ...
    *cost = 0;
    if (enc_mb.valid[mode] && (transform8x8 == 0 || mode != 0 || (mode == 0 && active_sps->direct_8x8_inference_flag)))
    {
      ...
      {
#ifdef MB32X32_MVC
        if (input->mv_competition > 0)// skip_mode is for P_SKIP..and here P_SKIP will never happen coz this is 8x8
          skip_mode = FALSE;
#endif
  ...
  for (rerun=0; rerun<runs; rerun++)
  {
    ...
    if (!intra)
    {
      ...
      if ((!inter_skip) && enc_mb.valid[P8x8])
      {
        ...
        if (input->RCEnable)
#ifdef MB32X32
          rc_store_diff64(img->opix_x,img->opix_y,img->mpr);
#else
