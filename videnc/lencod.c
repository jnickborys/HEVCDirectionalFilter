...
void init_img()
{
...
#endif
  {
    ...
    if(input->UseExtMB > 0)
    {
      ...
      {
        get_mem_mv (&(img->all16_mv8[i]));
        get_mem_mv (&(img->all16_mv4[i]));

        get_mem_mv (&(img->bipred_mv1_8[i]));
        get_mem_mv (&(img->bipred_mv2_8[i]));
        get_mem_mv (&(img->bipred_mv1_4[i]));
        get_mem_mv (&(img->bipred_mv2_4[i]));
      }
      for(i=0; i<5; i++)
      {
        get_mem_mv (&(img->all32_mv8[i]));
        get_mem_mv (&(img->all32_mv4[i]));
        get_mem_mv (&(img->bipred32_mv1_8[i]));
        get_mem_mv (&(img->bipred32_mv2_8[i]));
        get_mem_mv (&(img->bipred32_mv1_4[i]));
        get_mem_mv (&(img->bipred32_mv2_4[i]));
      }
    }
    else
    {
...
void free_img ()
{
...
#endif
  {
    ...
    if(input->UseExtMB > 0)
    {
      ...
      for(i=0; i<(1<<(2*MAX_MB_EXT_LEVEL)); i++)
      {
        free_mem_mv (img->all16_mv8[i]);
        free_mem_mv (img->all16_mv4[i]);
        free_mem_mv (img->bipred_mv1_8[i]);
        free_mem_mv (img->bipred_mv2_8[i]);
        free_mem_mv (img->bipred_mv1_4[i]);
        free_mem_mv (img->bipred_mv2_4[i]);
      }
      for(i=0; i<5; i++)
      {
        free_mem_mv (img->all32_mv8[i]);
        free_mem_mv (img->all32_mv4[i]);
        free_mem_mv (img->bipred32_mv1_8[i]);
        free_mem_mv (img->bipred32_mv2_8[i]);
        free_mem_mv (img->bipred32_mv1_4[i]);
        free_mem_mv (img->bipred32_mv2_4[i]);
      }
    }
    else
    {
...
void report()
{
  ...
  if ((p_log=fopen("log.dat","r"))==0)                      // check if file exist
  {
    ...
    else                                            // Create header for new log file
    {
      ...
      fprintf(p_log," ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- \n");
      fprintf(p_log,"|    ver      | Date  | Time  |         Sequence             | #Img |P/MbInt| QPI| QPP| QPB| Format  |Iperiod| #B | FMES | Hdmd | S.R |#Ref | Freq |Coding|RD-opt|Intra upd|8x8Tr|AIF|FDIF|APEC|AQM|AQP|MVRes|MVQ| SNRY 1| SNRU 1| SNRV 1| SNRY N| SNRU N| SNRV N|#Bitr I|#Bitr P|#Bitr B|#Bitr IPB|     Total Time   |      Me Time     |\n");
      fprintf(p_log," ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- \n");
      //FDIF - Fast Decision on Interpolation Filter
    }
  }
  else
  {
...
#ifdef ADAPTIVE_FILTER
  fprintf(p_log," %d |", input->UseAdaptiveFilter);
  fprintf(p_log," %d |", input->InterpolationDecision);
#else
  fprintf(p_log," 0 |");
#endif
#ifdef ADAPTIVE_FD_SD_CODING
  fprintf(p_log," %d  |", input->APEC_in_FD_and_SD);
#else
  fprintf(p_log," 0 |");
#endif
  
#ifdef ADAPTIVE_QUANTIZATION
  fprintf(p_log," %d |", input->UseAdaptiveQuantMatrix);
#else
  fprintf(p_log," 0 |");
#endif
#ifdef EIGHTH_PEL
  fprintf(p_log," 1/%d |", input->mv_res?8:4);
#else
  fprintf(p_log," 1/4 |");   
#endif
#ifdef MV_COMPETITION
  fprintf(p_log," %d |", input->mv_competition);
#else
  fprintf(p_log," 0 |");   
#endif
...
#ifdef EIGHTH_PEL
  printf(" Motion vector resolution          : 1/%i-pel\n",input->mv_res ? 8 : 4 );
#endif
#ifdef ADAPTIVE_FILTER
  printf(" Adaptive Filter                   : %i\n",input->UseAdaptiveFilter);
#endif
#ifdef ADAPTIVE_FD_SD_CODING
  printf(" Adaptive prediction error coding  : %i\n",input->APEC_in_FD_and_SD);
#endif
#ifdef ADAPTIVE_QUANTIZATION
  printf(" Adaptive quantization matrix      : %i\n",input->UseAdaptiveQuantMatrix);
#endif
#ifdef MV_COMPETITION
  printf(" MV Competition                    : %i",input->mv_competition);
  if (input->mv_competition > 0)
  {
    printf("\tPredictors for Skip: %s\n", input->predictors_skip);
    printf("\t\t\t\t\tPredictors for P vectors: %s\n", input->predictors_mvp);
    if (input->successive_Bframe>0)
      printf("\t\t\t\t\tPredictors for B vectors: %s\n", input->predictors_mvb);
  }
  else
    printf("\n");
#endif
...
#endif
#ifdef ADAPTIVE_LOOP_FILTER
  printf(" Adaptive Loop Filter              : %i\n",input->UseAdaptiveLoopFilter);
#endif
...
#ifdef ADAPTIVE_FILTER
    printf("-----------------------------------------------------------------------------------\n");    
    printf("  Frame  Bit/pic    QP   SnrY    SnrU    SnrV    Time(ms) MET(ms) Frm/Fld Ref AIF \n");
    printf("-----------------------------------------------------------------------------------\n");
#else
    printf("-----------------------------------------------------------------------------------\n");    
...
int init_global_buffers(void)
{
  ...
  if(input->UseExtMB > 0)
  {
    memory_size += get_mem2Dint(&img->cofAC16x16,2,257);
    memory_size += get_mem3Dint(&img->cofAC16x8, 2, 2, 129);
    memory_size += get_mem3Dint(&img->cofAC8x16, 2, 2, 129);
  }
#endif
...
#ifdef ADAPTIVE_QUANTIZATION
    if(input->UseAdaptiveQuantMatrix && input->FMEnable != 3)
      memory_size += EPZSInit();
#endif
...
void free_global_buffers(void)
{
  ...
  if(input->UseExtMB > 0)
  {
    free_mem2Dint(img->cofAC16x16);
    free_mem3Dint(img->cofAC16x8, 2);
    free_mem3Dint(img->cofAC8x16, 2);
  }
#endif
...
#ifdef ADAPTIVE_FD_SD_CODING
  free_mem2D((byte**)rddata_FDSD_coding.ipredmode);
  free_mem2D((byte**)rddata_FDSD_coding_interlace.ipredmode);
#endif
...
int get_mem_mv (short ******* mv)
{
  ...
  if(input->UseExtMB > 0)
  {
    if ((*mv = (short******)calloc(4<<(input->UseExtMB),sizeof(short*****))) == NULL)
      no_mem_exit ("get_mem_mv: mv");
    for (i=0; i<(4<<(input->UseExtMB)); i++)
    {
      if (((*mv)[i] = (short*****)calloc(4<<(input->UseExtMB),sizeof(short****))) == NULL)
        no_mem_exit ("get_mem_mv: mv");
      for (j=0; j<(4<<(input->UseExtMB)); j++)
      {
    ...
    return (4<<(input->UseExtMB))*(4<<(input->UseExtMB))*img->max_num_references*9*2*sizeof(short);
  }
  else
  {
#endif
...
************************************************************************
* \brief
*    For checking input parameters for AQMS 
************************************************************************
*/
int ParamCheck()
{
  // limitation
  if ( ( input->UseAdaptiveQuantMatrix ) && (input->rdopt==0) )
  {
    printf("\nERROR : When RDOptimization is not equal to 1,  AQMS is not supported. \n\n");
    return -1;
  }
  
  if ( ( input->UseAdaptiveQuantMatrix ) && (input->PicInterlace>0 || input->MbInterlace>0) )
  {
    printf("\nERROR : AQMS doesn't support Interlace Coding now. \n\n");
    return -1;
  }
...
************************************************************************
* \brief
*    For initializing quantization matrix param
************************************************************************
*/
void InitAdaptiveQuantParam()
{
  int i=0;
  for(i=0;i<6;i++) SkipRatio[i]=0.0;
  
  dwSkipMbCount = 0;
  dwSkipEnableMbCount = 0;

  img->mb_iaqms_idx = 0;
  if(input->UseAdaptiveQuantMatrix)
  {
    if(VALUE_OF_QM_PARAM0 != 32 || VALUE_OF_QM_PARAM1 != 16)
      img->slice_mqm_signaling_flag = 1;
    else
      img->slice_mqm_signaling_flag = 0;
  }
}
#endif
