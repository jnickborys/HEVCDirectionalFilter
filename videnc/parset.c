...
void GenerateParameterSets (void)
{
  ...
  {
    gdwAQMS_frame_num = 4;
    PicParSet[PPSID_AQMS] = AllocPPS();
    GeneratePictureParameterSet( PicParSet[PPSID_AQMS], sps, 0, input->WeightedPrediction, input->WeightedBiprediction, input->cb_qp_index_offset, input->cr_qp_index_offset);
  }
#endif
  active_sps = sps;
                                  ...
                                  )
{
  ...
  sps->constrained_set0_flag = (Boolean)0;
  sps->constrained_set1_flag = (Boolean)0;
  sps->constrained_set2_flag = (Boolean)0;
  sps->constrained_set3_flag = (Boolean)0;
  ...
  sps->delta_pic_order_always_zero_flag = (Boolean)img->delta_pic_order_always_zero_flag;
  ...
  sps->vui_parameters_present_flag = (Boolean)(input->rgb_input_flag && input->yuv_format==3);
...
#ifdef MB32X32
  len+=ue_v ("PPS: mb_ext_level",                    input->UseExtMB,                      bitstream);
#endif
