...
#ifdef ADAPTIVE_LOOP_FILTER
  get_mem1Dint(&(s->alf.coeff), MAX_NUM_COEF);
  get_mem1Dint(&(s->alf.coeff_chroma), MAX_NUM_COEF_C);
  if(((s->alf_blk_data) = (AlfBlock *) calloc(img->num_alf_block, sizeof(AlfBlock))) == NULL)
    no_mem_exit("alloc_storable_picture: s->alf_blk_data");
#endif
