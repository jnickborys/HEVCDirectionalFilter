 ...
 ************************************************************************
 */
void img2buf (imgpel** imgX, unsigned char* buf, int size_x, int size_y, int symbol_size_in_bytes, int crop_left, int crop_right, int crop_top, int crop_bottom, int output_bit_depth, int bit_depth_increase)
{
  int i,j;
...

  {
      ...
      case 1:
        {
          for(i=crop_top;i<size_y-crop_bottom;i++)
            ...
            {
              ui8 = (unsigned char) (bit_depth_increase==0)? imgX[i][j] : Clip3(0,((1<<output_bit_depth)-1),((imgX[i][j]+(1<<(bit_depth_increase-1)))>>bit_depth_increase));
      ...
      case 2:
        {
          for(i=crop_top;i<size_y-crop_bottom;i++)
            for(j=crop_left;j<size_x-crop_right;j++)
            {
              tmp16 = (unsigned short) (bit_depth_increase==0)? imgX[i][j] : Clip3(0,((1<<output_bit_depth)-1),((imgX[i][j]+(1<<(bit_depth_increase-1)))>>bit_depth_increase));
      ...
      case 4:
        {
          for(i=crop_top;i<size_y-crop_bottom;i++)
            for(j=crop_left;j<size_x-crop_right;j++)
            {
              tmp32 = (unsigned long) (bit_depth_increase==0)? imgX[i][j] : Clip3(0,((1<<output_bit_depth)-1),((imgX[i][j]+(1<<(bit_depth_increase-1)))>>bit_depth_increase));
    ...
    else
    {
      ...
      for(i=crop_top;i<size_y-crop_bottom;i++)
        for(j=crop_left;j<size_x-crop_right;j++)
        {
          imgpel tmp = (imgpel) (bit_depth_increase==0)? imgX[i][j] : Clip3(0,((1<<output_bit_depth)-1),((imgX[i][j]+(1<<(bit_depth_increase-1)))>>bit_depth_increase));
...
#ifdef  INTERNAL_BIT_DEPTH_INCREASE
    img2buf (p->imgUV[1], buf, p->size_x_cr, p->size_y_cr, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom, input->output_bitdepth, img->BitDepthIncreaseChroma);
#else
...
#ifdef  INTERNAL_BIT_DEPTH_INCREASE
  img2buf (p->imgY, buf, p->size_x, p->size_y, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom, input->output_bitdepth, img->BitDepthIncrease);
#else
...
#ifdef  INTERNAL_BIT_DEPTH_INCREASE
    img2buf (p->imgUV[0], buf, p->size_x_cr, p->size_y_cr, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom, input->output_bitdepth, img->BitDepthIncreaseChroma);
#else
...
#ifdef  INTERNAL_BIT_DEPTH_INCREASE
      img2buf (p->imgUV[0], buf, p->size_x/2, p->size_y/2, symbol_size_in_bytes, crop_left/2, crop_right/2, crop_top/2, crop_bottom/2, input->output_bitdepth, img->BitDepthIncreaseChroma);
#else
      // fake out U=V=128 to make a YUV 4:2:0 stream
