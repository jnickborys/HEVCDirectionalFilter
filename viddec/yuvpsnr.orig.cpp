...
int main(int argc,char **argv) {
    ...
    for (i = 0; i < iFrames; i++) {
    	...
    	// Format, Image stores Y ssamples, then U samples, then V samples
    	// Frame Size of U,V is 1/4 of Y
    	// Y samples

        ifs1.read((char *)pcImgBuf1, iImageArea/2);
        ifs2.read((char *)pcImgBuf1, iImageArea/2);
        tmp[0] = GetPSNR(pcImgBuf1, pcImgBuf2, iImageArea/2);

    	// U samples
        ifs1.read((char *)pcImgBuf1, iImageArea/4);
        ifs2.read((char *)pcImgBuf1, iImageArea/4);
