...
#ifdef MB32X32
    img->mb_ext_level = ue_v ("PPS: PPS: mb_ext_level"                , s);
#endif  
...
void activate_sps (seq_parameter_set_rbsp_t *sps)
{
  if (active_sps != sps)
  {
    ...
    img->BitDepthIncrease = (input->output_bitdepth!=-1)? img->bitdepth_luma - input->output_bitdepth : 0;
    img->BitDepthIncreaseChroma = (input->output_bitdepth!=-1)? img->bitdepth_chroma - input->output_bitdepth : 0;
#endif
...
#ifdef ADAPTIVE_FILTER
  img->UseAdaptiveFilter   = pps->adaptive_filter_flag;
#endif
}
